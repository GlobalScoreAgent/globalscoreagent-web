# Documentación en Español
Esta carpeta contiene **toda la documentación oficial** del proyecto **Global Score Agent** traducida al español.
Aquí encontrarás guías de negocio, especificaciones técnicas y explicaciones claras sobre los principales componentes de la plataforma.

---
## Índice de Documentos

### Estándares Fundamentales
- **[ERC-8004: Agentes Sin Confianza Previa](./erc-8004.md)**  
  Explicación del estándar de Ethereum que habilita la identidad, reputación y validación de agentes de IA sin confianza previa. (Negocio + Técnico)

### Documentos Principales
- **[Global Score Agent – Plataforma de Reputación para ERC-8004](./global-score-agent.md)**  
  Descripción general del proyecto, misión, productos y visión.
- **[Precios de Global Score Agent (GSA)](./gsa-pricing.md)**  
  Planes del Dashboard, paquetes de créditos API y preguntas frecuentes.

### API Pública
- **[API Pública – Free Tier](./public-api-free-tier.md)**  
  Documentación de los endpoints públicos gratuitos: Búsqueda de Agentes y Nivel de Madurez. Incluye ejemplos de uso, buenas prácticas y detalles técnicos. Ideal para desarrolladores y agentes autónomos.

### Índices de Reputación
- **[Índice HUMI – Agent Human-like Intelligence Index](./index-humi.md)**  
  Explicación completa del puntaje de reputación de los Agentes.
- **[Índice WAMI – Wallet Advanced Metrics Index](./index-wami.md)**  
  Explicación completa del puntaje de reputación de las Wallets.

### Análisis y Evaluaciones
- **[Análisis de Realness del Agente](./agent-realness-analysis.md)**  
  Cómo evaluamos la legitimidad y calidad básica de cada Agente.
- **[Puntaje de Riqueza de Metadatos](./agent-metadata-richness-analysis.md)**  
  Cómo medimos la completitud y profesionalismo del perfil de un Agente.
- **[Sistema de Advertencias de Agentes](./agent-warning-system.md)**  
  Tipos de advertencias, su significado y cómo afectan la reputación.
- **[Tipos de Feedback de Agentes](./agent-feedback-types.md)**  
  Explicación de los diferentes tipos de feedback (attestations, comments, audits, etc.).
- **[Categorías Transaccionales de Wallets](./wallet-transactional-categories.md)**  
  Cómo clasificamos el comportamiento de las wallets (Explosive, Hyper Growth, Stable, etc.).

---
## ¿Cómo usar esta documentación?

- **Usuarios y Creadores**: Empieza por **ERC-8004** y **Global Score Agent** para entender la base del ecosistema y cómo mejorar tu Agente.
- **Desarrolladores y Agentes Autónomos**: Revisa la documentación de la **API Pública – Free Tier** y las especificaciones técnicas de **HUMI** y **WAMI**.
- **Inversores y Analistas**: Usa **Precios** y los índices de reputación para evaluar oportunidades.

---
## Contribuciones

Esta documentación se mantiene actualizada por el equipo de Global Score Agent.  
Si encuentras errores, quieres proponer mejoras o agregar nuevos documentos, por favor abre un Issue o Pull Request en el repositorio.

---
**Última actualización:** Junio 2026  
**Versión:** 1.2

---
**¿Listo para explorar?**
- [Ir al Dashboard](https://www.globalscoreagent.com)
- [Ver Demo en Vivo](https://www.globalscoreagent.com/demo)
- [Contactar al equipo](mailto:hello@globalscoreagent.com)
