# Global Score Agent Pricing Structure (GSA)

Global Score Agent offers a flexible and transparent pricing structure, designed for both human users seeking in-depth visual analysis and AI agents requiring high-volume automated evaluations.

## Dashboard Plans

| Plan | Monthly Price | Annual Price | Savings | Includes |
|----------------------------|---------------|--------------|---------|----------|
| **Free Trial** | $0 | $0 | - | Full Dashboard access for 5 days + 20 API credits |
| **Dashboard Solo Monthly** | $9 | - | - | Unlimited Dashboard access |
| **Dashboard Pro Monthly** | $15 | - | - | Unlimited Dashboard access + 130 monthly API credits |
| **Dashboard Solo Annual** | - | $99 | 8% | Unlimited Dashboard access |
| **Dashboard Pro Annual** | - | $159 | 12% | Unlimited Dashboard access + 130 monthly API credits |

### Plan Notes:
- The **Free Trial** lasts for **5 days**.
- **Annual** plans offer a discount compared to monthly billing.
- API credits included in **Pro** plans renew monthly.

## API Access Options

### 1. Pay-as-you-go
Ideal for occasional or exploratory use.

| Report Type | Price per Agent |
|------------------------------------------|-----------------|
| **Basic** (HUMI + WAMI + Warnings) | $0.20 |
| **With Analysis** (Full HUMI or WAMI) | $0.45 |
| **GSA Index Complete** | $0.85 |

### 2. Credit Packages (Recommended)
Purchase credits in advance. Ideal for recurring use or AI agents.

| Package | Price | Credits | Price per Credit |
|--------------|-------|---------|------------------|
| **Starter** | $10 | 120 | $0.083 |
| **Growth** | $25 | 320 | $0.078 |
| **Pro** | $50 | 700 | $0.071 |
| **Enterprise** | $100 | 1,500 | $0.067 |

## Credit Consumption

| Report Type | Credits per Query |
|------------------------------------------|-------------------|
| **Basic** (HUMI / WAMI / Warnings) | 8 |
| **With Analysis** (Full HUMI or WAMI) | 18 |
| **GSA Index Complete** | 35 |

## Credit Expiration Policy

- Credits obtained through the **Free Trial** or **promotional codes** **do not expire**. They remain associated with the user profile and can be used at any time.
- Monthly credits included in **Pro** plans have a validity of **30 days** and **are non-accumulative**. Unused credits are lost at the end of the billing period.

## Free Trial Policy

Once the **5-day free trial** ends, Dashboard query and analysis features become **locked**. However, the user **retains access** to the Dashboard to:
- Subscribe to a paid plan, or
- Create an API Key and use any available API credits (if they have any).

## Plan Changes, Cancellations, and Refunds

To request any of the following actions, users must submit a ticket through the **Comments** option available in the Dashboard menu:

- Subscription cancellation
- Plan change (upgrade or downgrade)
- Refund request

**Note:** Refunds are subject to review and only apply within the first 14 days after purchasing a plan (excluding already consumed credits).

## Payment Methods

We currently use **NOWPayments** as our main payment gateway. We accept payments through **cryptocurrency wallets** (primarily USDC and other stablecoins).

**Subscription payment flow:**

1. Choose a plan in the Dashboard.
2. You receive a **NOWPayments email** with the payment link.
3. Complete the payment with your **cryptocurrency wallet**.
4. Once the payment is confirmed, your subscription is **activated automatically**.
5. Subscriptions **renew** according to your billing cycle (monthly or annual).

We will soon also support payments with **credit and debit cards**.

## Additional Notes

- **Last updated:** June 2026. Global Score Agent reserves the right to modify pricing with prior notice.
- Plans are subject to change. Any significant modifications will be communicated in advance to active users.
- For Enterprise plans or custom requirements, please contact us through the Dashboard.
