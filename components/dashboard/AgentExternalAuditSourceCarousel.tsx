...
import {
  formatExternalSourceScore,
  humanizeExternalEntity,
  type ExternalSourceRow,
} from '@/lib/agentExternalAuditSummary';
...
export function AgentExternalAuditSourceCarousel({
  rows,
  isDark,
  t,
  resetKey,
  ...
  const sectionHeader = (counter: string | null) => (
    <div className="mb-4 flex items-center justify-between gap-2">
      <h3 className={sectionTitleClass}>{t.agentDetailExternalAuditSourcesTitle}</h3>
      {counter ? (
  ...
  if (rows.length === 0) {
    return (
      <div>
        {sectionHeader(null)}
        <p className={`text-sm text-center ${muted}`}>{t.agentDetailExternalAuditEmpty}</p>
      </div>
    );
  }
  ...
  const entityLabel = humanizeExternalEntity(activeRow.entity);
      ...
      <div className="flex items-stretch gap-2">
        {canNavigate ? (
          <button
            ...
            onClick={goPrev}
            aria-label={t.agentDetailExternalAuditPrev}
          >
        ...
        >
          <div className="flex flex-wrap items-center justify-center gap-1.5">
            <span className={`text-xs ${muted}`}>{t.agentDetailExternalAuditEntityLabel}:</span>
            <span
              className={cn(
          ...
          <div className="mt-4 text-center">
            <p className={`text-xs ${muted}`}>{t.agentDetailExternalAuditScoreLabel}</p>
            <p className="mt-1 text-2xl font-bold tabular-nums">
              {formatExternalSourceScore(activeRow.score)}
            </p>
          </div>

          <div className="mt-4 text-center">
            <p className={`text-xs font-semibold uppercase tracking-wide ${muted}`}>
              {t.agentDetailExternalAuditLastAtLabel}
            </p>
        ...
        {canNavigate ? (
          <button
            ...
            className={navBtnClass}
            onClick={goNext}
            aria-label={t.agentDetailExternalAuditNext}
          >
        ...
        <div className="mt-3 flex justify-center gap-1.5">
          {rows.map((row, i) => (
            <button
              key={`${row.entity}-${i}`}
              type="button"
              aria-label={`${humanizeExternalEntity(row.entity)} (${i + 1}/${rows.length})`}
