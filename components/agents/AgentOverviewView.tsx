...
import {
  getRealnessDisplayTier,
  getRealnessStatusBusinessMeaning,
  getRealnessStatusColor,
  getRealnessStatusLabel,
  normalizeRealnessScore,
  parseRealnessStatus,
...
import { useAgentRecentNavigation } from '@/app/(dashboard)/dashboard/components/AgentRecentNavigationContext';
...
export function AgentOverviewView({ routeScope }: { routeScope: AgentRouteScope }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(agentDetailApiPath(routeId, lookupBy, '', routeScope), {
          credentials: routeScope === 'dashboard' ? 'include' : 'same-origin',
        });
        const body = await res.json().catch(() => ({}));
        if (cancelled) return;
        if (res.status === 401 && routeScope === 'dashboard') {
          window.location.href = buildAuthLoginUrl(agentDetailPagePath(routeId, lookupBy, routeScope));
          return;
        }
        if (!res.ok || !body?.data) {
          setError(t.agentDetailLoadError);
  ...
  }, [routeId, lookupBy, routeScope, t.agentDetailLoadError]);
  ...
  useEffect(() => {
    if (routeScope !== 'dashboard' || !agent) return;
    recordAgentVisit(String(agent.id ?? routeId), String(agent.name ?? routeId));
  }, [agent, recordAgentVisit, routeId, routeScope]);
  ...
  }, [agent, routeId, t.agentOverviewTitle, setTitleOverride, routeScope]);
  ...
  const chainLogoSrc = useMemo(
    () =>
      publicChainLogoUrl(
        resolveChainLogoFileName(
          agent?.chain_logo_file_name as string | null | undefined,
          typeof agent?.chain_name === 'string' ? agent.chain_name : null,
        ),
      ),
    [agent?.chain_logo_file_name, agent?.chain_name],
  );
  ...
  const realnessStatus = parseRealnessStatus(agent?.realness_status);
  const realnessScore = normalizeRealnessScore(agent?.realness_score);
  const realnessTier = getRealnessDisplayTier(realnessStatus);
  const realnessColor = getRealnessStatusColor(realnessStatus);
  const realnessStatusLabel = getRealnessStatusLabel(realnessStatus, t);
  const realnessBadgeHelpText = getRealnessStatusBusinessMeaning(realnessStatus, t);
                ...
                <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <AgentDetailIndexScoreCard
                    ...
                    detailsHref={withPublicLang(
                      agentDetailSubPagePath(routeId, 'humi', lookupBy, routeScope),
                    )}
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-12 xl:items-stretch">
          <div className="flex h-full flex-col gap-4 self-stretch xl:col-span-5">
            <AgentDetailOnChainCard
              ...
              realnessTier={realnessTier}
              realnessStatusLabel={realnessStatusLabel}
              realnessAccentColor={realnessColor}
              realnessBadgeHelpText={realnessBadgeHelpText}
            />
