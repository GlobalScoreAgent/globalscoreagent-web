...
import { Landmark, PieChart, Activity, Globe2, RotateCcw } from 'lucide-react';
import { useLanguage } from '@/app/contexts/LanguageContext';
import { wamiCopy, type WamiPillar } from '@/content/wami/copy';
...
type WamiPillarFlipCardProps = {
  pillar: WamiPillar;
  isFlipped: boolean;
  onFlip: () => void;
};

export default function WamiPillarFlipCard({ pillar, isFlipped, onFlip }: WamiPillarFlipCardProps) {
  const { language } = useLanguage();
  const Icon = iconMap[pillar.id];
  const { flipHint, flipBack, pointsLabel } = wamiCopy.pillars;
